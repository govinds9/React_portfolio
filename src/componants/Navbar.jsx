...
const Navbar=()=>{
    ...
    const list = arr.map(prod=>(
        <li  key = {prod.id}className="px-4 font-medium cursor-pointer text-gray-500 hover:scale-125 duration-200"><Link to={prod.part} smooth duration={1000}> {prod.part}</Link></li>
    )

    );
    const list1= arr.map(prod=>(
        <li  key = {prod.id}className="py-6 text-4xl font-medium cursor-pointer text-gray-500 hover:scale-125 duration-200"><Link onClick={()=>setnav(!nav)} to={prod.part}  smooth duration={1000}> {prod.part}</Link></li>
    )
